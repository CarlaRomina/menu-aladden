...
const ItemMenu = ({ posicion, menu, setMenu }) => {
      ...
      if (result.isConfirmed) {
        consultaBorrarMenu(menu._id).then((respuesta) => {
          ...
          if (respuesta.status === 200) {
            Swal.fire("Menú eliminado", `El ${menu.nombreProducto} fue eliminado correctamente`, "success");

            consultaListaMenu().then((respuesta) => setMenu(respuesta));
          } else {
            Swal.fire("Ocurrio un error", `Intente realizar esta operación nuevamente mas tarde`, "error");
          }
        });
      }
    });
  ...
  return (
    <tr>
      ...
      <td>
        <Link className="btn btn-warning" to={"editar-menu/" + menu._id}>
          Editar
        </Link>
